    ...
    fetch("http://localhost:8000/api/createuser", {
      ...
      .then((data) => {
        if (data.success === false) {
          if (
            data.message.email &&
            data.message.email.includes("Email already exists.")
          ) {
            alert("Email already exists.");
          } else {
            // Handle other potential error messages
            alert(
              data.message.email ? data.message.email[0] : "Signup failed!"
            );
          }
        } else {
          console.log("Success:", data);
          alert("Signup successful!, please enter your details to login");
          navigate("/");
        }
      })
      .catch((error) => {
